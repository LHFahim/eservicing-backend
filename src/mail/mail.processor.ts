...
export class MailProcessor {
    ...
    constructor(private readonly appConfig: ConfigService, private mailProvider: IEMailProvider) {}
    ...
    async sendEmailVerificationEmail(job: Job<{ email: string; token: string }>): Promise<any> {
    ...
    async sendResetPasswordEmail(job: Job<{ email: string; token: string }>): Promise<any> {
    ...
    @Process(Jobs.emailAddressChangeEmail)
    async emailAddressChangeEmail(job: Job<{ email: string; token: string }>): Promise<any> {
        this.logger.log(`Sending email address change email to '${job.data.email}'`);

        const { email, token } = job.data;
        try {
            const subject = 'Starter project | Email Address Change';
            ...
            this.logger.error(`Failed to send email address change email to '${email}'`, error.stack);
            throw error;
        }
    }
    ...
    async sendAccountCreatedEmail(
        ...
        try {
            const subject = 'Starter project | Welcome to our site';
            const payload = await this.buildEmailPayload(subject, email, {
            ...
            this.logger.error(`Failed to send account created email to '${email}'`, error.stack);
            throw error;
        }
    }

    private async buildEmailPayload(subject: string, recipient: string, data: EmailTemplates) {
        const source = await fs.readFile(`email_templates/${data.template}`, 'utf8');
