...
import { GoogleOAuthResponse, GoogleProfileResponse } from '../types/google.type';
...
export class GoogleOAuthProvider implements OAuthServiceProvider {
    ...
    async getUserInfo(accessToken: string): Promise<OAuthProfileResponse> {
        const details = await this.googleAPI.get<GoogleProfileResponse>('oauth2/v2/userinfo', {
        ...
        return { avatarURL: picture, isEmailVerified: verified_email, firstName, lastName, email };
    }
    ...
    getRedirectURL() {
        ...
        url.searchParams.set('state', Base64Encode(JSON.stringify({ provider: OAuthProviders.GOOGLE })));
