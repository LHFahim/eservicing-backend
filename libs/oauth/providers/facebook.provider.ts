...
import { FacebookOAuthResponse, FacebookProfileResponse } from '../types/facebook.type';
...
export class FacebookOAuthProvider implements OAuthServiceProvider {
    ...
    private getCodeValidationURL(code: string) {
        return `oauth/access_token?client_id=${this.config.facebook.clientID}&redirect_uri=${this.config.redirectURL}&client_secret=${this.config.facebook.clientSecret}&code=${code}`;
    }

    async validateCode(code: string): Promise<string> {
        const response2 = await this.http.get<FacebookOAuthResponse>(this.getCodeValidationURL(code));
    ...
    async getUserInfo(accessToken: string): Promise<OAuthProfileResponse> {
        ...
        const { first_name: firstName, last_name: lastName, email, picture } = details;
    ...
    getRedirectURL() {
        ...
        url.searchParams.set('client_id', this.config.facebook.clientID);

        url.searchParams.set('response_type', 'code');
        url.searchParams.set('scope', 'email,public_profile');
        url.searchParams.set('state', Base64Encode(JSON.stringify({ provider: OAuthProviders.FACEBOOK })));
